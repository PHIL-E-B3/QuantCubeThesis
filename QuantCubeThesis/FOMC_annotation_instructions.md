...
| `macro` | Aggregate economic outlook, overall trajectory, multi-variable assessments where no single variable dominates. Use when the sentence references "economic conditions", "economic outlook", "economic developments", or similar aggregate phrases **without specifying a single dominant variable** (e.g., GDP, output, consumption). Do not conflate with `economic_activity` — `macro` is for unspecified aggregate framing. |
...
- `boilerplate` examples: "The Committee voted to approve...", "No decisions were made at the meeting", legal/regulatory language, section headers like "Staff Review of the Financial Situation", memorial tributes. Also applies to formulaic monitoring/evaluation language that recurs across statements without substantive policy content, e.g., "The Committee will continue to evaluate the timing and overall amounts of its purchases in light of the evolving economic outlook." When in doubt, ask: does this sentence communicate a specific policy signal or economic assessment, or is it a standard procedural hedge? If the latter, use `boilerplate`.
...
| Label | When to use |
|-------|-------------|
| `hawkish` | Signals tightening, rate increase, tapering, or balance sheet reduction. Also applies when a dual-mandate sentence explicitly references returning inflation to target while inflation is above target — the inflation framing dominates and signals willingness to tighten. |
...
| Label | When to use |
|-------|-------------|
| `near_term` | Explicit short-term horizon or imminent action ("at this meeting", "in coming months"). Also use for `unconditional` actions that have already been taken or announced with no stated long-run duration — the implicit horizon is immediate/near-term. |
...
| `["economic_activity"]` | Policy conditioned on specific growth/output measures (GDP, consumption, investment, etc.) |
| `["macro"]` | Policy conditioned on aggregate economic conditions or outlook without naming a specific variable — e.g., "economic conditions", "economic outlook", "economic developments" |
...
Otherwise `dom = "na"`. Do not infer dominance from context, ordering, or historical knowledge — it must be explicitly stated in the sentence itself.
...
| `skewed_upside` | Sentence explicitly frames risks toward better-than-expected or inflation-overshoot outcomes. Key phrases: "upside risk", "risks to inflation are to the upside". Also applies when language signals asymmetric vigilance about inflation even without the exact phrase — e.g., "monitoring inflation developments closely" in a context where overall risks are stated as balanced implies the inflation tail is the active concern. **Note: for inflation, upside risk = more inflation = bad for policy = `skewed_upside` is still labeled as such. The sign convention is handled in regression, not in the label.** |
...
con:  ["inflation"] | ["unemployment"] | ["economic_activity"] | ["macro"] | ["financial_conditions"] | ["none"] | "na"
